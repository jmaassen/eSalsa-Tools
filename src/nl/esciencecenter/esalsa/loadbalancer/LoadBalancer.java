...
/**
 * 
 * LoadBalancer is used to generate a block distribution for the Parallel Ocean Program (POP). This block distribution is based on
 * the ocean topography, the desired block size, and the desired number of clusters, nodes per cluster, and cores per node.
 * 
...
public class LoadBalancer {
    ...
    /** The width of topography on which the grid is defined */
    private final int topographyWidth;

    /** The width of topography on which the grid is defined */
    private final int topographyHeight;
     ...
     * 
     * @param layers
     *            the store for the layers
     * @param neighbours
     *            the function object used to determine the block neighbors.
     * @param grid
     *            the grid containing all blocks.
     * @param blockWidth
     ...
     * @throws Exception
     *             if the LoadBalancer could not be initialized.
     */
    public LoadBalancer(Layers layers, int topographyWidth, int topographyHeight, Grid grid, int blockWidth, int blockHeight,
        ...
        System.out.println("ALL: " + allBlocks.size());
        System.out.println("BLOCK: " + blockLayer.size());
     ...
     * @param output
     *            the output layer in which to store the subsets.
     * @throws Exception
     *             if the split failed.
     */
    private void split(Set set, int subsets, Layer output) throws Exception {
        ...
        if (splitMethod.equalsIgnoreCase("simple")) {
            ...
            split = new SearchSplit(set, subsets);
        }
     ...
     * 
     * @param previousLayer
     *            the name of the existing layer to split.
     * @param newLayer
     *            the name of the new layer to create.
     * @param subsets
     *            the number of subsets to create.
     * @throws Exception
     *             if the subsets could not be created.
     */
    ...
    public void printStatistics(String layer) throws Exception {
        new Statistics(layers).printStatistics(layer, System.out);
    }
     ...
     * 
     * @param set
     *            the set containing the blocks to mark.
     * @param value
     *            the value to mark the blocks with.
     */
     ...
     * 
     * - Initially all blocks in the combined layer are marked with 0. - Next, we call {@link #divideWork(Set)} to assign all work
     * in the subsets of the combined set.
     * 
     ...
     * As a result:
     * 
     * - all blocks in a CORE set will end up with the same mark - all blocks in NODE set will have marks ranging from (X ..
     * X+coresPerNode), - all blocks in a CLUSTER set will have marks ranging from (Y ... Y+(corePerNode*nodesPerCLuster))
     * 
     */
     ...
     * @throws Exception
     *             the blocks could not be distributed.
     */
        ...
        for (Set s : layer) {
            ...
            for (Block b : s) {
                if (b.blockID > 0) { 
                    ...
                    result[c.y * grid.width + c.x] = b.getMark() + 1;
                }
            }
        }
